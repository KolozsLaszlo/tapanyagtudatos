      ...
      {!selectedRecipe ? (
        <>
          ...
          {!loading && !error && recipes.length === 0 && searchQuery && (
            <div className="text-center text-gray-500">Nincs találat!</div>
          )}
            ...
            {recipes.map((recipe) => (
              ...
              >
                ...
                <p className="text-gray-600 mt-2">
                  <strong>Hozzávalók:</strong> {recipe.strIngredient1},{" "}
                  {recipe.strIngredient2}
                </p>
                <p className="text-gray-700 mt-2">
                  {recipe.strInstructions.length > 50
                    ? recipe.strInstructions.substring(0, 50) + "... tovább"
        ...
        <div className="bg-white p-6 rounded-lg shadow-md max-w-3xl mx-auto flex flex-col min-h-screen">
          ...
          <p className="text-gray-700 mt-2">
            <strong>Hozzávalók:</strong> {selectedRecipe.strIngredient1},{" "}
          ...
          {selectedRecipe.strCookTime && (
            <p className="text-gray-700 mt-2">
              <strong>Elkészítési idő:</strong> {selectedRecipe.strCookTime}{" "}
              perc
            </p>
          )}
          <button
            onClick={handleBackToSearch}
            className="mt-5 p-2 text-white rounded self-center w-full max-w-xs"
          >
            Vissza
          </button>
        </div>
